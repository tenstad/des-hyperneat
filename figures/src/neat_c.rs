...
pub fn create() -> figure::Figure {
    ...
    let mut edge =
        fig.edge_builder(&|e| e.width(0.08).pos(0.5).text_color("blue").color("black"));
    edge(&n1, &n5, &|e| e.text("\\raisebox{-1em}{8}").pos(0.3).text_pos("right"));
    ...
    edge(&n3, &n5, &|e| e.text("\\raisebox{-1em}{9}").pos(0.3).text_pos("left"));
